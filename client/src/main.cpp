...
void printHelp()
{
    ...
    std::cout << "  connect <id>      - Connect to a peer (P2P)" << std::endl;
    ...
    std::cout << "\n--- Relay Mode ---" << std::endl;
    std::cout << "  relayauth <pwd>   - Authenticate for relay mode" << std::endl;
    std::cout << "  relayconnect <id> - Connect to peer via relay" << std::endl;
    std::cout << "  relaysend <id> <msg> - Send via relay" << std::endl;
    std::cout << "  relaypeers        - List relay connected peers" << std::endl;
    std::cout << "  relaydisconnect <id> - Disconnect relay" << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    client.setOnRelayAuthResult([](bool success, const std::string& message) {
        if (success) {
            std::cout << "\n[Relay] Authentication successful: " << message << std::endl;
        } else {
            std::cout << "\n[Relay] Authentication failed: " << message << std::endl;
        }
    });

    client.setOnRelayConnected([](const std::string& peerId) {
        std::cout << "\n[Relay] Connected to: " << peerId << std::endl;
    });

    client.setOnRelayDisconnected([](const std::string& peerId) {
        std::cout << "\n[Relay] Disconnected from: " << peerId << std::endl;
    });
    ...
    while (std::getline(std::cin, line))
    {
        ...
        else if (command == "relayauth")
        {
            if (tokens.size() >= 2)
            {
                std::string password = tokens[1];
                if (client.authenticateRelay(password))
                {
                    std::cout << "Relay authentication successful!" << std::endl;
                }
                else
                {
                    std::cout << "Relay authentication failed" << std::endl;
                }
            }
            else
            {
                std::cout << "Usage: relayauth <password>" << std::endl;
            }
        }
        else if (command == "relayconnect")
        {
            if (tokens.size() >= 2)
            {
                if (client.connectToPeerViaRelay(tokens[1]))
                {
                    std::cout << "Relay connected to " << tokens[1] << std::endl;
                }
            }
            else
            {
                std::cout << "Usage: relayconnect <peer_id>" << std::endl;
            }
        }
        else if (command == "relaysend")
        {
            if (tokens.size() >= 3)
            {
                std::string targetPeer = tokens[1];
                ...
                if (msgStart != std::string::npos)
                {
                    ...
                    {
                        std::string message = line.substr(msgStart);
                        if (client.sendTextViaRelay(targetPeer, message))
                        {
                            std::cout << "Relay sent to " << targetPeer << ": " << message << std::endl;
                        }
                    }
                }
            }
            ...
            {
                std::cout << "Usage: relaysend <peer_id> <message>" << std::endl;
            }
        }
        else if (command == "relaypeers")
        {
            auto peers = client.getRelayConnectedPeers();
            std::cout << "Relay connected peers (" << peers.size() << "):" << std::endl;
            for (const auto &p : peers)
            {
                std::cout << "  - " << p << std::endl;
            }
        }
        else if (command == "relaydisconnect")
        {
            if (tokens.size() >= 2)
            {
                client.disconnectFromPeerViaRelay(tokens[1]);
                std::cout << "Relay disconnected from " << tokens[1] << std::endl;
            }
            else
            {
                std::cout << "Usage: relaydisconnect <peer_id>" << std::endl;
            }
        }
